...
class JoomleagueConnector extends JLCalendar{
  ...
  function formatBirthdays( $rows, &$matches, $dates ) {
    ...
    foreach ($rows AS $key => $row) {
      ...
      $newrows[$key]['team'] = '';
      global $mainframe;
      if ($this->xparams->get('jlbirthdaypix', 0) == 1 AND $row->default_picture != '' AND file_exists($mainframe->getCfg('absolute_path').DIRECTORY_SEPARATOR.str_replace('/', DS, $row->default_picture))) {
        $linkit = 1;
