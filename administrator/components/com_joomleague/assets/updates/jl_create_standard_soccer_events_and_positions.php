...
function addStandardsForSoccer()
{
	...
	if ('AddEvents' == 'AddEvents')
	{
		...
		if (!$object=$db->loadObject())
		{
			$query					= build_InsertQuery_Event('eventtype',$newEventName,$newEventIcon,$sports_type->id,11); $db->setQuery($query);
			$result					= $db->query();
		...
		if (!$object=$db->loadObject())
		{
			$query					= build_InsertQuery_Event('eventtype',$newEventName,$newEventIcon,$sports_type->id,12); $db->setQuery($query);
			$result					= $db->query();
