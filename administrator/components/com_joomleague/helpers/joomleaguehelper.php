...
if( !defined('THUMBLIB_BASE_PATH') ) {
	require_once(JLG_PATH_SITE.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.'PHPThumb'.DIRECTORY_SEPARATOR.'ThumbLib.inc.php');
}
...
class JoomleagueHelper
{
	...
	public static function getExtensions($project_id)
	{
		...
		if(JFolder::exists(JPATH_SITE.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_joomleague'.DIRECTORY_SEPARATOR.'extensions')) {
			$folderExtensions  = JFolder::folders(JPATH_SITE.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_joomleague'.DIRECTORY_SEPARATOR.'extensions',
...
// Set vars to check if thumbnailcreation is needed
			list($source_width, $source_height) = getimagesize(JPath::clean(JPATH_SITE.DIRECTORY_SEPARATOR.str_replace(JPATH_SITE.DS, '', $picture)));
			$needthumb=1;
...
// check if the cache-directory exitst if not create one
					...
					$image_cache_path=JPATH::clean(JPATH_SITE.DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'joomleague'.DIRECTORY_SEPARATOR.$image_path_parts[dirname]);
...
// check if there is a chached actual image if not, create one
					...
					$cached_thumb=JPATH::clean(JPATH_SITE.DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'joomleague'.DIRECTORY_SEPARATOR.$image_path_parts[dirname].DIRECTORY_SEPARATOR.$image_timestamp.'_'.$height.'_'.$width.'_'.$image_path_parts[filename].'.'.$format);
...
// Check if there is are older files. If Yes, delete them.
					$matches = glob(JPATH::clean(JPATH_SITE.DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'joomleague'.DIRECTORY_SEPARATOR.$image_path_parts[dirname].DIRECTORY_SEPARATOR.'*_'.$height.'_'.$width.'_'.$image_path_parts[filename].'*'));
...
// return cached or uncached (if not necessary) images
	...
	public static function getMatchEndTimestamp($match, $totalMatchDuration, $format = 'Y-m-d H:i')
	{
		...
		if ($match->match_date)
		{
			$start = new DateTime($match->match_date->format('Y-m-d H:i'), new DateTimeZone($match->timezone));
			$end = $start->add(new DateInterval('P'.$totalMatchDuration.'M'));
			$endTimestamp = $end->format($format, true);
		}
		return $endTimestamp;
	}
