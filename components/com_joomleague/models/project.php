...
class JoomleagueModelProject extends JModelLegacy
{
	...
	function getTemplateConfig($template)
	{
		...
		foreach ($extensions as $e => $extension) {
			...
			$paramsdata="";
			$strXmlFile=$JLGPATH_EXTENSION.DIRECTORY_SEPARATOR.'settings'.DIRECTORY_SEPARATOR."default".DIRECTORY_SEPARATOR.$template.'.xml';
			if(file_exists($JLGPATH_EXTENSION.DIRECTORY_SEPARATOR.'settings'.DIRECTORY_SEPARATOR."default".DIRECTORY_SEPARATOR.$template.'.xml')) {
				$form = JForm::getInstance($template, $strXmlFile);
				$fieldsets = $form->getFieldsets();
				foreach ($fieldsets as $fieldset) {
					foreach($form->getFieldset($fieldset->name) as $field) {
