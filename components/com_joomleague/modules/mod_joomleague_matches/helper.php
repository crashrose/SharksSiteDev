...
class modMatchesHelper {
	...
	public function getpix($team) {
		...
		if($pt=="country" && strlen($countrycode)==3) {
			...
			$matchpart_pic = (!empty ($team-> $pt) AND file_exists(JPATH_ROOT .DIRECTORY_SEPARATOR. $team-> $pt)) ? $team-> $pt : $defaultlogos[$pt];
			if (file_exists(JPATH_ROOT .DIRECTORY_SEPARATOR. $matchpart_pic)) {
				$size = getimagesize(JPATH_ROOT .DIRECTORY_SEPARATOR. $matchpart_pic);
				$pic_width = $size[0];
				$pic_height = $size[1];
			...
			$pic['src'] = (trim($matchpart_pic) != "" && file_exists(JPATH_ROOT .DIRECTORY_SEPARATOR. trim($matchpart_pic))) ? JUri :: root(true) .
